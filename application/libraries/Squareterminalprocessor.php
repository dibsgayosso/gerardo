...
class Squareterminalprocessor extends Creditcardprocessor
{
	...
	public function do_start_cc_processing()
	{
		try
		{
			...
			{							
				...
				if (isset($errors))
				{
					$this->controller->_reload_for_next_request(array('error' => lang('common_error')), false);
					return;
				}
				...
				if ($api_response->isSuccess() && $response->getStatus() == 'COMPLETED' && count($square_checkout_payment_ids) > 0 && in_array($square_payment->getStatus(),array('APPROVED','COMPLETED')))
				{
					@$TextResponse = '';
				   ...
				   $CCExpire = $square_payment->getCardDetails()->getCard()->getExpMonth().'/'.$square_payment->getCardDetails()->getCard()->getExpYear();
					...
					if ($Authorize == $Purchase)
					{
						...
						{							
							...
							$this->controller->_reload_for_next_request(array('warning' => lang('sales_credit_card_partially_charged_please_complete_sale_with_another_payment_method')), false);			
						}
					}
			...
			{
				$this->controller->_reload_for_next_request(array('error' => lang('sales_must_return_based_on_previous_sale')), false);
			}
		}
		catch(Exception $e)
		{
	...
	public function cancel_cc_processing()
	{
		$this->controller->cart->delete_payment($this->controller->cart->get_payment_ids(lang('common_credit')));
		$this->controller->cart->save();
		$this->controller->_reload_for_next_request(array('error' => lang('sales_cc_processing_cancelled')), false);
	}
