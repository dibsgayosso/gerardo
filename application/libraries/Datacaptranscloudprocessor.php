...
abstract class Datacaptranscloudprocessor extends Creditcardprocessor
{
    ...
    @$TextResponse = $response->RStream->TextResponse;
		...
		if ($CmdStatus == 'Approved')
		{
			...
			if ($Authorize == abs($Purchase))
			{
				...
				else
				{
					...
					$this->controller->cart->save();
					$this->controller->_reload_for_next_request(array('warning' => lang('sales_gift_card_partially_charged_please_complete_sale_with_another_payment_method')));
				}
			}
		...
		{
			$this->controller->_reload_for_next_request(array('error' => $CmdStatus.': '.$TextResponse));
		}
	}
	...
	{	
		...
		elseif($customer_info->cc_token)
		{
			if ($cc_amount <= 0)
			{
				$this->controller->_reload_for_next_request(array('error' => lang('sales_charging_card_failed_please_try_again')));
				return;
			}
	   ...
	   $TextResponse = $response->RStream->TextResponse;
		   ...
		   $ProcessData = @$response->RStream->ProcessData;
			...
			if ($Authorize == $Purchase)
			{
				...
				else //Change payment type to Partial Credit Card and show sales interface
				{
					...
					$this->pad_reset();
					$this->controller->_reload_for_next_request(array('warning' => lang('sales_credit_card_partially_charged_please_complete_sale_with_another_payment_method')));			
				}
			}
			elseif($Authorize < $Purchase)
			{
		...
		else
		{
			...
			{
				$this->controller->session->set_userdata('CC_SUCCESS', FALSE);
				$this->controller->session->set_userdata('CC_DECLINE', TRUE);
				return;
			}
			else
			{
				$this->controller->_reload_for_next_request(array('error' => $CmdStatus.': '.urldecode($TextResponse)));
			}
		}
	}
	...
	public function cancel_cc_processing()
	{
		$this->controller->cart->delete_payment($this->controller->cart->get_payment_ids(lang('common_credit')));
		$this->controller->cart->save();
		$this->controller->_reload_for_next_request(array('error' => lang('sales_cc_processing_cancelled')));
	}
