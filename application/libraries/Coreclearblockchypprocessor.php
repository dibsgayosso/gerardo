		...
		else
		{
			...
			if ($response === NULL)
			{
				try
				{
					...
					{
						$this->controller->_reload_for_next_request(array('error' => lang('sales_unable_to_determine_transaction_status_please_check_coreclear')));
					}
					else
					{
						$this->controller->_reload_for_next_request(array('error' => lang('sales_terminal_connection_failed_please_try_again')));
					}
				}
				catch(Exception $e)
				{
			...
			{
				$this->controller->_reload_for_next_request(array('error' => $TextResponse));
			}
