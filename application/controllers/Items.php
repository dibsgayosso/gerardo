...
class Items extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function excel_export_start($include_location_columns = 0,$create_temp_file = 1)
	{
		$this->load->view('excel_export_start',array('export_url' => site_url('items/excel_export/'.($include_location_columns ? 1 : 0).'/'.($create_temp_file ? 1 : 0)),'status_check_url' => site_url('items/get_recent_export_file_url')));
	}
	
	function get_recent_export_file_url()
	{
		$recent_excel_export_file_id = $this->session->userdata('recent_excel_export_file_id');
		
		if ($recent_excel_export_file_id)
		{		
			echo json_encode(array('url' => site_url('home/download/'.$recent_excel_export_file_id.'/1')));	
		}
		else
		{
	...
	function excel_export($include_location_columns = 0,$create_temp_file = 0) 
	{			
		...
		if ($create_temp_file)
		{
			array_to_spreadsheet($rows,'items_export.'.$extension, FALSE, 'items_excel_export_'.date('Y-m-d-h-i-s').'.'.$extension);
		}
		else
		{
