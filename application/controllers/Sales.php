...
class Sales extends Secure_area
{
	...
	function check_processing_status()
	{
		...
		{
			echo json_encode(array('url' => site_url('sales/complete')));	
		}
		elseif ($this->session->userdata('CC_DECLINE'))
		{
			echo json_encode(array('url' => site_url('sales/declined')));	
		}
		elseif($this->session->userdata('reload_data'))
		{
			echo json_encode(array('url' => site_url('sales/reload_for_next_request')));	
		}
		else
		{
	...
	function reload_for_next_request()
	{
		$data = $this->session->userdata('reload_data') ? $this->session->userdata('reload_data') : array();
		$this->_reload($data,false);
		$this->session->unset_userdata('reload_data');
		
	}
	
	function _reload_for_next_request($data=array())
	{
		$this->session->set_userdata('reload_data',$data);
	}
